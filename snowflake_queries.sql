...
-- This query creates an external stage named "ext_stage." An external stage is used to interact with external data sources, in this case, an 
-- S3 bucket ("s3://coinmarketcap-bucket/real_time_data/") where real-time cryptocurrency data is stored. The credentials for accessing the
--  S3 bucket are provided in the query using AWS_KEY_ID and AWS_SECRET_KEY.

-- In the SQL query you provided, a stage named ext_stage is being created or replaced. In the context of Snowflake, a stage is an object used for
--  data loading and unloading operations to and from external data sources, typically cloud storage platforms like Amazon S3, Azure Blob Storage, or 
--  Google Cloud Storage. 

-- Stage is like a connection from data source for loading and unloading data